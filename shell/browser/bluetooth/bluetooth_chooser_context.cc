...
WebBluetoothDeviceId BluetoothChooserContext::GetWebBluetoothDeviceId(
  ...
  const std::vector<std::unique_ptr<base::Value>> device_list =
      permission_manager->GetGrantedDevices(
          static_cast<blink::PermissionType>(
              WebContentsPermissionHelper::PermissionType::BLUETOOTH),
          origin, browser_context_);
...
WebBluetoothDeviceId BluetoothChooserContext::GrantServiceAccessPermission(
  ...
  // |device_address|, update the allowed GATT services by performing a union
  // of |services|.
  auto* permission_manager = static_cast<ElectronPermissionManager*>(
      browser_context_->GetPermissionControllerDelegate());
  const std::vector<std::unique_ptr<base::Value>> device_list =
  ...
  for (const auto& device_object : device_list) {
    if (device_address == *device_object->FindStringKey(kDeviceAddressKey)) {
      ...
      permission_manager->RevokeDevicePermission(
          static_cast<blink::PermissionType>(
              WebContentsPermissionHelper::PermissionType::BLUETOOTH),
          origin, *device_object, browser_context_);
      permission_manager->GrantDevicePermission(
          static_cast<blink::PermissionType>(
              WebContentsPermissionHelper::PermissionType::BLUETOOTH),
          origin, std::move(new_device_object), browser_context_);
      return device_id;
    }
  }

  // If the device has been detected through the Web Bluetooth Scanning API,
  // grant permission using the WebBluetoothDeviceId generated through that
  // API. Remove the ID from the temporary |scanned_devices_| map to avoid
  ...
  permission_manager->GrantDevicePermission(
      static_cast<blink::PermissionType>(
          WebContentsPermissionHelper::PermissionType::BLUETOOTH),
      origin, std::move(permission_object), browser_context_);
  return device_id;
}
...
void BluetoothChooserContext::RevokeDevicePermissionWebInitiated(
  ...
  if (!device.is_none()) {
    ...
    permission_manager->RevokeDevicePermission(
        static_cast<blink::PermissionType>(
            WebContentsPermissionHelper::PermissionType::BLUETOOTH),
        origin, device, browser_context_);
  }
}
...
BluetoothChooserContext::GetGrantedObjects(const url::Origin& origin) {
  auto* permission_manager = static_cast<ElectronPermissionManager*>(
      browser_context_->GetPermissionControllerDelegate());
  return permission_manager->GetGrantedDevices(
      static_cast<blink::PermissionType>(
          WebContentsPermissionHelper::PermissionType::BLUETOOTH),
      origin, browser_context_);
}
